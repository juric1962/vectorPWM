...
extern uint8_t WriteStack[MAX_WRITE_STACK][MIN_LEN_BUFFER];
...
void scenario(void)
{
  ...
  if (oneSec) {oneSec=0;CountSecWaitOK++; wd_sec++;HAL_GPIO_TogglePin(GPIOD, GPIO_PIN_3);}
       if(wd_sec>10) {
                      wd_sec=0;
                      if(work_bit==0) {reset_ESP();comand_WiFi=0;return;}
                      else work_bit=0;
       }
...
//   p=copy_P((unsigned char *)&aga.f,(char const __flash *)&plo[2].f);    // из структуры flash в сруктуру memory
  ...
  {    
    case 0:         
      ...
      puk=(char*)&rs232TxBuf[0];//At_com.buf[0];            
      sprintf(puk,"AT\r\n\0");
      puk=(char*)&rs232TxBuf[0];//&At_com.buf[0];
      while(*puk)puk++;
      rs232TxPtr=puk-(char*)&rs232TxBuf[0];//&At_com.buf[0];      
      comand_WiFi=1; // ожидание ответа           
      HAL_UART_Transmit_DMA(&huart3,(uint8_t*)&rs232TxBuf,rs232TxPtr);      
      debugOutput((uint8_t*)&rs232TxBuf,rs232TxPtr);
    break;   
    case 1:        
    if(waitForWiFiFlag&&(wifiTimeOutTimer==0)){comand_WiFi=0;return;}  
    if(rs232DataReady){
      rs232DataReady=0;  
      debugOutput((uint8_t*)&rs232RxBuf,rs232RxPtr);
      count_ri=rs232RxPtr;//At_com.cnt_rx;
      if((rs232RxBuf[count_ri-1]==0xa)&&
         (rs232RxBuf[count_ri-2]==0xd)&&
         (rs232RxBuf[count_ri-3]=='K')&&
           (rs232RxBuf[count_ri-4]=='O') ){/*fl_at_com.tm_out=0;*/comand_WiFi=2;return;}
   ...
   ///////// next comamnd ATE0
    case 2:       
       ...
       puk=(char*)&rs232TxBuf[0];//&At_com.buf[0];
       while(*puk) puk++;
       rs232TxPtr=puk-(char*)&rs232TxBuf[0];
    ...
    if(rs232DataReady){
      ...
      if((rs232RxBuf[count_ri-1]==0xa)&&
         (rs232RxBuf[count_ri-2]==0xd)&&
         (rs232RxBuf[count_ri-3]=='K')&&
         (rs232RxBuf[count_ri-4]=='O') ){comand_WiFi=4;return;}
    ...
    case 5:       
      ...
      if(rs232DataReady){
        ...
        if((rs232RxBuf[count_ri-1]==0xa)&&
           (rs232RxBuf[count_ri-2]==0xd)&&
           (rs232RxBuf[count_ri-3]=='K')&&
           (rs232RxBuf[count_ri-4]=='O') ){comand_WiFi=6;return;}
        else {comand_WiFi=0;return;} // нет ответа на команду повторить ее      
      }
    return;   
    ...
    case 7:
      ...
      if(rs232DataReady){
        ...
        if((rs232RxBuf[count_ri-1]==0xa)&&
           (rs232RxBuf[count_ri-2]==0xd)&&
           (rs232RxBuf[count_ri-3]=='K')&&
           (rs232RxBuf[count_ri-4]=='O') ){comand_WiFi=8;return;}
        else {comand_WiFi=0;return;} // нет ответа на команду повторить ее      
      }
    return;    
   ...
   case 9:  
      ...
      if(rs232DataReady){
        ...
        if((rs232RxBuf[count_ri-1]==0xa)&&
           (rs232RxBuf[count_ri-2]==0xd)&&
           (rs232RxBuf[count_ri-3]=='K')&&
           (rs232RxBuf[count_ri-4]=='O') ){comand_WiFi=10;return;}
        else {comand_WiFi=0;return;} // нет ответа на команду повторить ее      
      }
    return;     
    ...
    case 11:    
      ...
      if(rs232DataReady){
        ...
        if((rs232RxBuf[count_ri-1]==0xa)&&
           (rs232RxBuf[count_ri-2]==0xd)&&
           (rs232RxBuf[count_ri-3]=='K')&&
           (rs232RxBuf[count_ri-4]=='O') ){comand_WiFi=12;return;}
        else {rs232RxBuf[rs232RxPtr]=0;comand_WiFi=10;/*0*/;return;} // нет ответа на команду повторить ее      
      }
    return;   
    ...
    case 13:
      ...
      if(rs232DataReady){
        ...
        if((rs232RxBuf[count_ri-1]==0xa)&&
           (rs232RxBuf[count_ri-2]==0xd)&&
           (rs232RxBuf[count_ri-3]=='K')&&
           (rs232RxBuf[count_ri-4]=='O') ){comand_WiFi=14;return;}
        else {comand_WiFi=0;return;} // нет ответа на команду повторить ее      
      }
    return;    
    ...
    case 115:    
      ...
      if(rs232DataReady){
        ...
        if((rs232RxBuf[count_ri-1]==0xa)&&
           (rs232RxBuf[count_ri-2]==0xd)&&
           (rs232RxBuf[count_ri-3]=='K')&&
           (rs232RxBuf[count_ri-4]=='O') ){comand_WiFi=116;return;}
        else {comand_WiFi=0;return;} // нет ответа на команду повторить ее      
      }
    return;    
   ...
   case 16:     
     ...
     if(rs232DataReady){
        ...
        count_ri=rs232RxPtr;  
        debugOutput((uint8_t*)&rs232RxBuf,rs232RxPtr);       
        rs232RxBuf[count_ri]=0; // конец строки
...
//**********************************************************************************
      ...
      for (ii=count_ri;ii> icc;ii--)
      {
        if((rs232RxBuf[ii]=='y')&&
           ...
           (rs232RxBuf[ii-2]=='a')&&  
           (rs232RxBuf[ii-3]=='e')&&  
           (rs232RxBuf[ii-4]=='r')){reset_ESP();comand_WiFi=0;return;}      
      }
      for (ii=count_ri;ii> icc;ii--)
      {
        if((rs232RxBuf[ii]  =='y')&&
           (rs232RxBuf[ii-1]=='s')&&  
           (rs232RxBuf[ii-2]=='u')&&  
           (rs232RxBuf[ii-3]=='b')){reset_ESP();comand_WiFi=0;return;}      
      }      
//**********************************************************************************      
      if (CreateHtmlItem((char*)rs232RxBuf,(unsigned int*)&rs232RxPtr,count_ri) == WRITE_PORT)          
      {
          HAL_UART_Transmit_DMA(&huart3,(uint8_t*)&rs232RxBuf,rs232RxPtr); 
          debugOutput((uint8_t*)&rs232RxBuf,rs232RxPtr);
          waitForWiFiFlag=0;
          rs232RxPtr=0;
          rs232DataReady=0; 
   ...
   case 17:
     ...
     if (IndexWriteStack >= CountWriteStack)
     {
       CountWriteStack = 0;
       IndexWriteStack = 0;
       comand_WiFi = 16;
     ...
     for (i = 0; i < MIN_LEN_BUFFER; i++)
        outBuffer[i] = WriteStack[IndexWriteStack][i];
     
     modbusRegistersEdit(outBuffer, inBuffer, 1, &byteSize);
     IndexWriteStack++;
   break; 
